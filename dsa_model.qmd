---
title: "Dangerous Shot Attempt (DSA) Model"
format: html
editor: visual
---
...
This is a so-called "expected goals" model built using XGBoost. My goal with this model is to have a way to identify a dangerous shot attempt ("**DSA**"). However, this model could be used as a general xG model.

Based on the data used to test this model, a potential threshold for a DSA is any shot attempt with a DSA value \>= 0.063. This would capture the top 40% of shot attempts (which account for roughly 3/4 of actual goals).
...
tuning_grid <- grid_latin_hypercube(trees(range = c(275, 500)),
                             tree_depth(range = c(2,3)),
                             finalize(mtry(), training_data_pp),
                             min_n(range = c(15, 35)),
                             sample_size = sample_prop(range = c(0.75, 0.95)),
                             size = 150)
...
#tuned_best_param <- tibble(trees = 319,
                           #mtry = 10,
                           #min_n = 27,
                           #tree_depth = 3,
                           #sample_size = 0.9465991)
...
saveRDS(xgb_fit_tuned, "dsa_model.rds")
