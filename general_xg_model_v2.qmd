...
This is an NHL expected goals (xG) model.

This model computes xG values for shots on goal. It does not compute xG values for missed shots (unlike many other public models) or shots on an empty net.

The model type is XGBoost.

The data used to fit the model go back to the 2017-2018 season but exclude the 2020-2021 season (COVID-19).

This model was last updated on May 16, 2024.
...
Start with all goals (excluding shootout goals).
...
Now exclude empty net goals which boost the third period but are not relevant to this xG model.
...
Show the breakdown of events occurring immediately before a goal.
...
Show the breakdown of events occurring two steps before a goal.
...
Zoom in on cross-ice plays that occur within 3 seconds of a previous event (and under 40 feet on the y-axis).
...
Repeat, but filter for when the lag event is a shot on goal.
...
xg_pp_function <- function(pbp_data) {
...
pbp_data_pp <- xg_pp_function(pbp_data)

saveRDS(xg_pp_function, "xg_pp_function.rds")
...
tuning_grid <- grid_latin_hypercube(trees(range = c(425, 750)),
                                    tree_depth(range = c(2,3)),
                                    finalize(mtry(), training_data),
                                    min_n(range = c(1, 20)),
                                    sample_size = sample_prop(range = c(0.8, 0.9)),
                                    size = 30)
...
#tuned_best_param <- tibble(trees = 573,
                           #mtry = 9,
                           #min_n = 11,
                           #tree_depth = 2,
                           #sample_size = 0.8073802)
