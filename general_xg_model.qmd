...
Load the play-by-play data if they're saved locally then jump to Join Play-By-Play Data below. Otherwise, load the NHL API function and pull the data.
...
split_data <- initial_split(data = pbp_data)
...
training_data_pp <- training_data %>%
...
xgb_folds <- vfold_cv(training_data_pp,
                      strata = event_type)

xgb_grid <- grid_latin_hypercube(trees(range = c(250, 2000)),
                                 tree_depth(range = c(1, 2)),
                                 finalize(mtry(), training_data_pp),
                                 min_n(range = c(10, 20)),
                                 sample_size = sample_prop(range = c(0.3, 0.9)),
                                 size = 50)
...
xgb_tuned <- tune_race_anova(xgb_wflow,
                             resamples = xgb_folds,
                             grid = xgb_grid)

show_best <- show_best(xgb_tuned,
                       metric = "roc_auc")
...
xgb_tuned_best <- tibble(trees = show_best$trees[1],
                         mtry = show_best$mtry[1],
                         min_n = show_best$min_n[1],
                         tree_depth = show_best$tree_depth[1],
                         sample_size = show_best$sample_size[1])

xgb_wflow_final <- xgb_wflow %>%
        finalize_workflow(xgb_tuned_best)

xgb_fit <- xgb_wflow_final %>%
...
xgoals_test_res <- predict(xgb_fit, 
                           new_data = testing_data_pp %>% select(-event_type),
                           type = "prob")

xgoals_test_res <- bind_cols(xgoals_test_res, 
