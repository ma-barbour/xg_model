...
xgb_recipe <- recipe(event_type ~ .,
                     data = training_data) %>%
        step_mutate(lag_event = lag(event_type),
                    lag_zone = lag(zone),
        ...
        step_mutate(shot_type = if_else(is.na(shot_type), "unknown", shot_type),
        ...
        step_mutate(lag_zone = as.factor(lag_zone)) %>%
        ...
        step_other(lag_event, 
                   threshold = 0.1, 
                   id = "le_id") %>%
        step_filter(event_type == "goal" | event_type == "shot-on-goal",
                    event_on_en == "FALSE",
                    period != 5) %>%
        step_select(event_type,
                    sa_distance,
                    sa_angle,
                    shot_type,
                    period,
...
#%>%
#step_interact(terms = ~ starts_with(event_type):y_diff:elapsed_time,
...
xgb_folds <- vfold_cv(training_data,
                      strata = event_type)
